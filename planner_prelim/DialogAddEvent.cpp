...
CDialogAddEvent::CDialogAddEvent(CWnd* pParent /*=NULL*/)
	: CDialogEx(IDD_ADD_EVENT, pParent)
{
...
bool CDialogAddEvent::CheckData()
{
	...
	{
		AfxMessageBox(_T("Description Time not entered."));
		return 0;
	}
	else return 1;
...
bool CDialogAddEvent::CheckTimes()
{
	...
	AfxMessageBox(_T("First one is good"));
...
void CDialogAddEvent::OnBnClickedOk()
{
	...
	if (CheckData()) {
		...
		CPlannerEvent* NewEvent = new CPlannerEvent(m_Description, &m_Place, m_BegDate, m_EndDate, m_BegDateMinutes, m_EndDateMinutes);

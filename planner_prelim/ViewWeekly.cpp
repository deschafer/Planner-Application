...
CViewWeekly::CViewWeekly(unsigned *Rows, unsigned *Columns, int *Height,
	...
	m_FirstMonthFirstWeek(0), m_LastMonthLastWeek(0)
{
...
void CViewWeekly::DrawReminder(CDC* pDC)
{
	...
	CRect aRect(CPoint(m_WidthPortion * 3, *m_Height - m_HeightPortion - 40), CPoint(m_WidthPortion * 4, *m_Height - 40));
...
void CViewWeekly::DrawWeeklyGrid(CDC* pDC)
{
	...
	for (int height = m_HeightPortion + *m_TopBarSize; height < *m_Height; height += m_HeightPortion, i++)
	{
		...
		if(i == 0) String.Format(L"%d a.m.", 12);
		else if (i < 12) String.Format(L"%d a.m.", i);
		else if (i == 12) String.Format(L"%d p.m.", 12);
		else String.Format(L"%d p.m.", i - 12);
		
		// Moving to correct position to print out time
		pDC->MoveTo(0, height );
		pDC->LineTo(*m_Width, height);
		pDC->TextOutW(0, height - m_HeightPortion, String);
	}
...
void CViewWeekly::DrawColumn(CPlannerView* View, CDC* pDC, int Date, int Day, CMonth* Month)
{
	...
	for (int CellHeight = *m_TopBarSize, n = 0 + m_Offset; CellHeight < *m_Height, (n) < 24; CellHeight += m_HeightPortion, n++)
	{
		...
		// Drawing today's events in this space
		if (Today->GetEventsForTime(n)->GetHead() != nullptr)
		{

			Today->GetEventsForTime(n)->p_EventAux = Today->GetEventsForTime(n)->GetHead();

			for (int j = 0; Today->GetEventsForTime(n)->p_EventAux != nullptr; Today->GetEventsForTime(n)->p_EventAux = Today->GetEventsForTime(n)->p_EventAux->next, j++)
			{
				if (Today->GetEventsForTime(n)->p_EventAux->Object.GetPlace() != nullptr)
				{
					Str.Format(L"%s : %s @ %s", *Today->GetEventsForTime(n)->p_EventAux->Object.GetDescription(),
						*Today->GetEventsForTime(n)->p_EventAux->Object.GetPlace(), Today->GetEventsForTime(n)->p_EventAux->Object.GetTimeStandardForm());
				}
				else
				{
					Str.Format(L"%s @ %s", *Today->GetEventsForTime(n)->p_EventAux->Object.GetDescription(),
						*Today->GetEventsForTime(n)->p_EventAux->Object.GetTimeStandardForm());
...
int CViewWeekly::MoveRow(int Movement)
{
	...
	{
		if(m_RowCounter < 14) m_RowCounter++;
		if (m_Offset < 14) m_Offset++;
...
int CViewWeekly::OnNextMonth() 
{
	...
	if (m_CurrentMonth->ReturnMonthType() == 11 && m_Planner->IsLastYear(m_Year->ReturnYearDate()) && m_LastMonthLastWeek);
	else
	{
		m_WeekStartDay++;
		...
		{
			OnNextYear();
			if (m_CurrentMonth->ReturnBegDate() != 0) m_WeekStartDay = 1;
			m_FirstWeek = 1;
		}
...
int CViewWeekly::OnPrevMonth()
{
	...
	// as there is not valid week to complete this operation
	if (m_CurrentMonth->ReturnMonthType() == 0 && m_Planner->IsStartYear(m_Year->ReturnYearDate()) && m_FirstMonthFirstWeek);
	else
	{
		m_WeekStartDay--;
		if (m_WeekStartDay < 0)
...
int CViewWeekly::OnPrevYear()
{
	...
	if (m_CurrentMonth->ReturnMonthType() == 0 && m_Planner->IsStartYear(m_CurrentMonth->ReturnDayWithDate(1)->GetYear()));
	else
	{
		int error = m_Planner->MoveCurrMonth(-1);
...
bool CViewWeekly::SetObject(CPoint Point, CDay& DayObject)
{
	...
	for (int i = 0; i < 7; i++)
	{
		if (Point.x > i * m_WidthPortion 
			...
			Point.y > *m_TopBarSize && m_Week[i] != nullptr)
		{
			m_SelectedDay = m_Week[i];
			DayObject = *m_Week[i];

			Row = ((Point.y - *m_TopBarSize) / m_HeightPortion) + m_Offset;
